...
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------#read in vphas data
input_fpath = os.getcwd()+'/cs_data.txt'
...
#add a distance error entry if there isn't one
for l in input_tab:
	if len(l)<len(input_titles):
		l.append(float('nan'))
...
for line in data:
	...
	# a, u of PTB_25 looks to have autoguider noise, but the photometry looks fine
	#if line['pn']=='PTB_25':
	#	if line['block']=='a':
	#		line['u'] = float('nan')
	...
	#Sh2-71_cs2 observations have different dates. r and i are on the 31st, and everything else is
	#on the 28th. This is a variable star so these observations shouldn't be combined.		
	#don't use c,g of sh 2-71 cs 2: its variable and the b and c pointings are ~year apart
	if line['pn'] == 'SH_2-71_cs2':
		if line['pointing_num']==113:
			if line['block'] =='c':
				line['g'] = float('nan')
				line['g_upper_lim'] = float('nan')
				line['g_lower_lim'] = float('nan')
			
			

	#PNG293, c, g, poor calibration
	if line['pn'] == 'PNG293.4+00.1':		
				
		#don't use 1841, it's in DR3
		if line['pointing_num'] == 1841:
			line['u'] = float('nan')
			line['u_upper_lim'] = float('nan')
			line['u_lower_lim'] = float('nan')
			...
			line['r_r'] = float('nan')
			line['r_r_upper_lim'] = float('nan')
			line['r_r_lower_lim'] = float('nan')
			
			line['r2'] = float('nan')
			line['r2_upper_lim'] = float('nan')
			line['r2_lower_lim'] = float('nan')

			line['i'] = float('nan')
			line['i_upper_lim'] = float('nan')
			line['i_lower_lim'] = float('nan')
					
		
		"""	
		#don't use 1841, block B u, g, r2  or C g for PNG293.4+00.1 as poor seeing
		if line['pointing_num'] == 1841:		
			if line['block'] =='b':
				line['u'] = float('nan')
				line['u_upper_lim'] = float('nan')
	...
	#PNG344, a g,r,i poor calibration, so u calibraion is likely poor
	if line['pn'] == 'PNG344.4+01.8':
		if line['pointing_num'] == 1111:
			if line['block'] =='a':
	...
	#Abell c, g, poor calibration
	if line['pn'] == 'Abell_48':
		if line['pointing_num'] == 136:
			if line['block'] =='c':
				line['g'] = float('nan')
				line['g_upper_lim'] = float('nan')
				line['g_lower_lim'] = float('nan')
	...
	"""		
	#don't use rr A PNG288.2 as poor seeing
	if line['pn'] == 'PNG288.2+00.4':
		if line['pointing_num'] == 1738:
			if line['block'] =='a':
				line['r_r'] = float('nan')
				line['r_r_upper_lim'] = float('nan')
				line['r_r_lower_lim'] = float('nan')
...
completed_pn = []
new_colnames = ['pn', 'u', 'u_upper_lim', 'u_lower_lim', 'g', 'g_upper_lim', 'g_lower_lim', 'r_r', 'r_r_upper_lim', 'r_r_lower_lim', 'r2', 'r2_upper_lim', 'r2_lower_lim', 'i', 'i_upper_lim', 'i_lower_lim', 'Teff', 'Distance', 'Distance_err']
...
#Add distance of 1kpc if there isn't a distance already
for line in data:
	if np.isnan( line['Distance']):
		line['Distance'] = 1.0
...
with open('input_mags_table.csv', 'w+') as f:
	...
	for ind,line in enumerate(data):
		...
		f.write(str(line['pn']) +'\t'+ str(line['u']) +'\t'+ str(u_err)+ '\t'+ str(line['g'])+'\t'+ str(g_err)+ '\t'+ str(line['r_r'])+'\t'+ str(r_r_err)+ '\t'+ str(line['r2'])+'\t'+ str(r2_err)+ '\t'+ str(line['r'])+'\t'+ str(r_err)+ '\t'+ str(line['i'])+'\t'+ str(i_err) +'\t'+ str(line['J'])+'\t'+ str(J_err)+ '\n')  
...
#keep only the true CS; the ones we want to plot
true_cs = ['Hf_38_cs2', 'NGC_6337', 'PTB_25', 'PNG293.4+00.1', 'SH_2-71_cs1', 'PNG288.2+00.4', 'PNG242.6-04.4']
newtab = []
for line in data:
	if line['pn'] in true_cs:
		newtab.append(line)
data = make_recarray( newtab, newnames)
...
for ind,line in enumerate(data):
	...
	#112kK rounded to 110kK	
	if line['pn']=='SH_2-71_cs1':
		line['Teff']=110000
	if line['pn']=='SH_2-71_cs2':
		line['Teff']=110000
		
	if np.isnan(line['Teff']):
		line['Teff'] = 100000
		
	

#adjust table if Teff=nan, assume error of 5000K
Teff_errs = [5000. for line in data]
data = append_table(data, Teff_errs, 'Teff_err', '>f4')
...
model_fpath = os.getcwd()+'/latex_vphas_CS_synthetic_colours_inc_atmos.tab'
...
for line in data:
	...
	if line['pn']=='SH_2-71_cs2': line['pn']='Sh2-71 CS 2'
	if line['pn']=='SH_2-71_cs1': line['pn']='Sh2-71 CS 1'
...
for name, x, y in zip(data['pn'], Teff, colour):
	...
	elif name=='Sh2-71 CS 1': plt.annotate(name, xy=(x-6800,y-0.02))
...
#
companion_mag_fpath= os.getcwd() + '/vphas_MS_synthetic_colours_uleak.tab'
