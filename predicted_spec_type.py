...
"""Predict the spectral type of a star based on its observed vphas magntiudes, optionally reddened"""
...
#loop thorugh all the options to fin the best match

spec_type_list = [B5, B8, A0, F0, F5, G0, K2, K5, M0, M2, M4 ]
distances = [1000]   #distances in pc
reddenings = [1.38]     #E(B-V)s

#[spectype, distance, reddening, in_u, in_g, in_r, in_i, E(B-V), dr_u, dr_g, dr_r, dr_i, avg_diff]
fintab = []


for choice in spec_type_list:
	for distance in distances:
		for reddening in reddenings:
			
			parameters = [choice['name'], distance, reddening]
			
			#add input vphas mags
			in_u = 19.412
			in_g = 18.941
			in_r = 18.015
			in_i = 17.871
				
			parameters.append(in_u) 
			parameters.append(in_g) 
			parameters.append(in_r) 
			parameters.append(in_i)
			...
			spect = [planck(w, choice['T']) for w in wavelengths] #W /m^3 /steradian
			...
			#1m^-2 = 1e-4 cm^-2
			spect = [val*1e7*1e-10*1e-4 for val in spect] #erg/s/cm2/A /ster


			#multiply by surface area to get the total flux of the star
			spect = [val * (4*math.pi*(choice['radius']*Rsun)**2) for val in spect]


			#calculate the total flux of the spectrum
			colnames = ['lambda', 'F_lambda']
			star_spectrum = make_recarray(zip(wavelengths_AA, spect), colnames)
			
			total_flux = calc_total_flux(star_spectrum) #erg/s

			#scale blackbody so the luminosity is what it should be     
			#expected_luminosity = luminosity     #(per sq cm)
			expected_lum = Lsun*choice['luminosity']    
			...
			avg_diff = ( abs(float(u)-in_u) + abs(float(g)-in_g) + abs(float(r)-in_r) + abs(float(i)-in_i) )/ 4
...
#order fintab by the nearest input magnitudes 
...
"""
savepath = os.getcwd()+'/sh2-71_cs2_mags_model.tab'
with open(savepath, 'w+') as f:
	f.write( 'wd	ms	distance	reddening	Sh_u	Sh_g	Sh_r	Sh_i	in_u	in_g	in_r	in_i	in_B	in_V	in_R	in_J	E(B-V)	dr_u	dr_g	dr_r	dr_i	Iexcess	comp_spectype	avg_diff' )
	f.write('\n')
	for line in sorted_list:
		txtline = ''
		for val in line:
